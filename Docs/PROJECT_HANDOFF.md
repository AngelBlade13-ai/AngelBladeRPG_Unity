...
At the time this document was pulled forward locally:
...
- Character-creation scene wiring and Play Mode testing are complete locally.
...
3. `feature/character-creation` — completed locally; ready to merge after commit/push
...
4. Confirm commit `97576e3` is present.
5. Open the project using Unity `6000.5.3f1`.
6. Open `Assets/Scenes/MainGameScene.unity`.
7. Confirm `CharacterCreationPanel` and its Inspector wiring are present.
8. Run the character-creation Play Mode checklist if the Unity version or scene changes.
9. Merge the branch into `main` after verification.
10. Create `test/core-gameplay-foundation` for the next milestone.
